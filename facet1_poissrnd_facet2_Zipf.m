...
degrees2=[];%定义一个空矩阵
for i=1:length(Nk)
    degrees2=[degrees2,ones(1,Nk(i))*i];%ones(1,Nk(i))生成1行Nk(i)列的全是1的矩阵。
end
degrees2=[degrees2,zeros(1,N-length(degrees2))];%缺失的点数为孤立点
%y = randsample([50:100],20) 返回一个向量，从整数 50 到 100 中无放回随机均匀抽取的 20 个值。
degrees_2=randsample(degrees2,length(degrees2));%生成的幂率度序列（从degrees1中不放回的抽取500个值放入degree中，相当于对degrees1中的数随机打乱）
K2=max(degrees_2);
%%
%可视化
figure;
...
load('facet1_poissrnd_distribution.mat');
%%
...
xlab=variance_k2/average_2;
save facet1=poissrnd_facet2=Zipf_3